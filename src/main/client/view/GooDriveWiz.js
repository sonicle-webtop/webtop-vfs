...
Ext.define('Sonicle.webtop.vfs.view.GooDriveWiz', {
	...
	createPages: function(path) {
		...
		return [{
			...
			items: [{
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s1.tit'),
				cls: 'x-window-header-title-default'
			}, {
				xtype: 'sospacer'
			}, {
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s1.txt')
			}]
		}, {
			itemId: 's2',
			xtype: 'wtwizardpage',
			items: [{
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s2.tit'),
				cls: 'x-window-header-title-default'
			}, {
				xtype: 'sospacer'
			}, {
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s2.txt')
			}, {
				xtype: 'wtform',
				items: [{
					xtype: 'solinkfield',
					...
					allowBlank: false,
					width: 420,
					emptyText: me.mys.res('gooDriveWiz.fld-code.tip'),
					fieldLabel: me.mys.res('gooDriveWiz.fld-code.lbl')
				}]
			}]
		}, {
			itemId: 's3',
			...
			items: [{
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s3.tit'),
				cls: 'x-window-header-title-default'
			}, {
				xtype: 'sospacer'
			}, {
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s3.txt')
			}, {
				xtype: 'sospacer'
			}, {
				xtype: 'wtform',
				...
				items: [{
					...
					allowBlank: false,
					width: 400,
					fieldLabel: me.mys.res('gooDriveWiz.fld-name.lbl')
				}]
			}]
		}, {
			itemId: 's4',
			...
			items: [{
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s4.tit'),
				cls: 'x-window-header-title-default'
			}, {
				xtype: 'sospacer'
			}, {
				xtype: 'label',
				html: me.mys.res('gooDriveWiz.s4.txt')
			}]
		}];
	},
	...
	onBeforeNavigate: function(s, dir, np, pp) {
		...
		if(pp === 's1') {			
			WT.ajaxReq(me.mys.ID, 'SetupStoreGoogleDrive', {
				params: {
					crud: 's1',
